...
function setIntersectT(g, edges, rox, opts){
    edges.forEach((e) => {
        ...
        let nodeW = g.node(e.w);
        let ind; 
        if(opts.rule == "ups"){ind = edges.indexOf(e)%(rox.length-2) + 1;}
        else{
    ...
    layer.n.forEach((v) => {
        g.edges().forEach((e) => { 
            if(e.v == v){
                ...
                else if(opts.rule == "ups" && g.node(e.v).y >= layer.m){ edges.push(e); }
                else if(opts.rule == "down" && g.node(e.v).y < layer.m){  edges.push(e); }
            }
        });
    });
