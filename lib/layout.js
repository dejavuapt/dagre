...
function logCurrentGraphState(g){
  console.log("----------------------------------- GRAPH LOGGING")
...
function assignNodeIntersects(g) {
  g.edges().forEach(e => {
    ...
    edge.points.push(util.intersectRect(nodeW, p2)); // вот тут есть вариант использования указания порта
    console.log(edge.points);
  });
}
...
function assignNodeIntersectsTView(g){
  ...
  for(let i = 0; i < layers.length/2; ++i){
    ...
    layer_data.ups.sort(function(v, w){return g.node(v).y - g.node(w).y});
    layer_data.dns.sort(function(v, w){return g.node(v).y - g.node(w).y}); 
    ...
    layers_data[i+1].dns.forEach((v) => {
      g.inEdges(v).forEach((e)=> {
        ...
        if(lnodes.find((p) => p == e.v)){ // TODO сделать так чтобы нормально было хз почему не работает
          let ind = (rox.length-2) - (layers_data[i+1].dns.indexOf(v)+1)%(rox.length - 2) + 1;
